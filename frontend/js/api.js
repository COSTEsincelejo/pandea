...
let WA_NUMBER = "573017056143";
async function loadWAConfig() {
  try {
    const r = await fetch(API_URL + "/api/config/whatsapp");
    const d = await r.json();
    WA_NUMBER = d.number;
  } catch(e) {}
}
function abrirWhatsApp(nombre, precio, cantidad = 1, talla = '') {
  // Mensaje profesional y claro con orden lógico: saludo, detalles del pedido y consulta de disponibilidad
  const tallaTexto = talla && talla.trim() ? talla : 'No especificada';
...
¿Está disponible? Agradecería información sobre tallas y colores disponibles.
Muchas gracias.`;
